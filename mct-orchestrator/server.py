...
MESH_API_URL = os.environ.get("MESH_CONTROLLER_URL", "https://mesh.local/api/v1").rstrip("/")
...
AUTOMATON_COMMAND_SCHEMA: dict[str, Any] = {
    ...
    "type": "object",
    "properties": {
        "automaton_id": {"type": "string", "minLength": 1, "maxLength": 128, "pattern": r"^[A-Za-z0-9._:-]+$"},
        "action": {"type": "string", "enum": ["initialize", "sync", "execute", "halt"]},
        "parameters": {
            "type": "object",
            "properties": {
                "target_node": {"type": "string", "minLength": 1, "maxLength": 128, "pattern": r"^[A-Za-z0-9._:-]+$"},
                "stability_threshold": {"type": "number", "minimum": 0.0, "maximum": 1.0},
            },
...
def _dispatch(command: dict[str, Any]) -> str:
    ...
    if errors:
        return _result(status="rejected", error="schema_validation_failed", details=[{"path": ".".join(map(str, error.path)), "message": error.message} for error in errors])
    ...
    endpoint = f"{mesh_api_url}/nodes/{quote(target_node, safe='')}/automata/{quote(automaton_id, safe='')}/commands"
    headers = {"Authorization": f"Bearer {security_token}", "Content-Type": "application/json", "Accept": "application/json", "User-Agent": "MCT-1700021-Orchestrator/1.0"}

    try:
        response = requests.post(endpoint, headers=headers, json={"action": action, "parameters": parameters}, timeout=timeout)
    except requests.Timeout:
        return _result(status="failed", error="mesh_timeout", target_node=target_node)
    except requests.ConnectionError as exc:
        return _result(status="failed", error="mesh_connection_failed", target_node=target_node, detail=str(exc))
    ...
    if not response.ok:
        return _result(status="failed", error="mesh_rejected_command", http_status=response.status_code, automaton_id=automaton_id, action=action, target_node=target_node, remote_result=remote_result)

    return _result(status="orchestrated", automaton_id=automaton_id, action_executed=action, target_node=target_node, http_status=response.status_code, dispatch_result=remote_result)


@orchestrator_mcp.tool()
def route_shb_request(request_payload_json: str) -> str:
    """Resolve a deterministic SHB route without executing the selected operation."""
    try:
    ...
    if not isinstance(capability, str) or not isinstance(operation, str) or isinstance(min_stability, bool) or not isinstance(min_stability, (int, float)):
        return _result(status="rejected", error="invalid_route_request")
    try:
        route = DEFAULT_ROUTER.resolve(capability=capability, operation=operation, min_stability=float(min_stability))
    except SHBRouteError as exc:
        return _result(status="rejected", error="no_safe_route", detail=str(exc))
    return _result(status="routed", route_id=route.route_id, target=route.target, capability=capability, operation=operation, stability=route.stability)
...
def dispatch_automaton_command(command_payload_json: str) -> str:
    """Dispatch a validated automaton command to a known node. This mutates an external system."""
    try:
